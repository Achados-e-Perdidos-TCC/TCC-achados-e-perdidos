...
Único mecanismo de login do app: **Bearer Token (JWT)**. (O login social via
Google/OAuth2 existiu numa versão anterior e foi removido do produto por
decisão do time.)

| Mecanismo | Como funciona | Onde vive o "login" |
|---|---|---|
| **Bearer Token (JWT)** | `POST /api/v1/auth/register` ou `/login` retornam um `accessToken` + `refreshToken` próprios, assinados com uma chave HS256 da aplicação. | Stateless — o cliente reenvia `Authorization: Bearer <token>` a cada requisição. Nenhuma sessão HTTP é criada. |
...
| `POST` | `/api/v1/auth/forgot-password` | `{ email }` | `200` `MessageResponse` | Nenhuma | Se o e-mail existir, envia por e-mail um link com token de redefinição. **Sempre** devolve a mesma mensagem genérica, exista ou não o e-mail (não dá pra usar a resposta para checar se um e-mail está cadastrado). |
...
`MessageResponse`: `{ message: string }` — corpo padrão dos endpoints que não devolvem `AuthResponse`, sempre com uma mensagem amigável em português, nunca um objeto de erro estruturado (esse é o `ApiError` do `GlobalExceptionHandler`, usado só nas respostas de erro — `400`/`401`).
...
- **Sem cookies/sessão**: a API é 100% stateless. Não é preciso `credentials:
  ...
  **⚠️ Isso só continua verdade enquanto o token nunca for guardado num
  cookie.** O backend desabilitou CSRF (`SecurityConfig`) partindo do princípio
  de que `accessToken`/`refreshToken` ficam em memória/`localStorage` e são
  anexados manualmente pelo JS a cada chamada — nunca enviados sozinhos pelo
  navegador. Se o front-end migrar para guardar o token em cookie (mesmo
  `httpOnly`, ex.: como mitigação de XSS), isso reabre uma vulnerabilidade de
  CSRF em todo endpoint que muda estado (criar item, trocar senha, excluir
  conta, etc.) — nesse cenário, avise o time de backend **antes** de mudar:
  o CSRF precisa voltar a ser ativado junto.
...
> A lista completa do que já foi entregue nesta refatoração está em
> `README.md`, seção "O que foi implementado nesta refatoração".
